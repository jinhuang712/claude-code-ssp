...
import { ZH_CATEGORY, descOf, nameOf, useStore } from "../store";
...
export function Picker() {
  ...
  return (
    <div className="drawer-backdrop" onClick={closePicker}>
      <aside className="drawer" onClick={(e) => e.stopPropagation()} role="dialog" aria-label="添加到状态栏">
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {list.map((w) => (
                  <button key={w.id} className="pick" onClick={() => addWidget(picker.line, picker.zone, w.id)}>
                    ...
                    <div className="hint mt-0.5">{descOf(w, w.id)}</div>
                  </button>
                ))}
              </div>
            </div>
