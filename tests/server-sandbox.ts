...
export interface Sandbox {
  root: string;
  /** Claude Code's config dir inside the sandbox (settings.json, plugins/claude-code-ssp/samples). */
  claudeDir: string;
...
export function enterSandbox(): Sandbox {
  ...
  const userConfig = path.join(root, "xdg", "claude-code-ssp", "config.json");
...
/** Write a captured-sample file the way `render` does, so /api/samples and ?cwd= have something to find. */
export function writeSample(claudeDir: string, sessionId: string, payload: Record<string, unknown>, capturedAt = Date.now()): string {
  const dir = path.join(claudeDir, "plugins", "claude-code-ssp", "samples");
